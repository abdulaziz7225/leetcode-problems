"""
Problem Number: 1385. Find the Distance Value Between Two Arrays
Difficulty Level: Easy
https://leetcode.com/problems/find-the-distance-value-between-two-arrays

********************************************************************************

Given two integer arrays arr1 and arr2, and the integer d, return the distance value
between the two arrays. The distance value is defined as the number of elements arr1[i]
such that there is not any element arr2[j] where |arr1[i]-arr2[j]| <= d.
  
Example 1:
Input: arr1 = [4,5,8], arr2 = [10,9,1,8], d = 2
Output: 2
Explanation: 
For arr1[0]=4 we have: 
|4-10|=6 > d=2 
|4-9|=5 > d=2 
|4-1|=3 > d=2 
|4-8|=4 > d=2 
For arr1[1]=5 we have: 
|5-10|=5 > d=2 
|5-9|=4 > d=2 
|5-1|=4 > d=2 
|5-8|=3 > d=2
For arr1[2]=8 we have:
|8-10|=2 <= d=2
|8-9|=1 <= d=2
|8-1|=7 > d=2
|8-8|=0 <= d=2

Example 2:
Input: arr1 = [1,4,2,3], arr2 = [-4,-3,6,10,20,30], d = 3
Output: 2

Example 3:
Input: arr1 = [2,1,100,3], arr2 = [-5,-2,10,-3,7], d = 6
Output: 1
  
Constraints:
1 <= arr1.length, arr2.length <= 500
-1000 <= arr1[i], arr2[j] <= 1000
0 <= d <= 100
"""

from typing import List


# Solution 1: Binary Search
class Solution:
    def findTheDistanceValue(self, arr1: List[int], arr2: List[int], d: int) -> int:
        n = len(arr2)
        arr2.sort()
        result = 0

        for num in arr1:
            left, right = 0, n - 1

            while left <= right:
                middle = (left + right) // 2

                if abs(num - arr2[middle]) <= d:
                    break
                elif arr2[middle] < num:
                    left = middle + 1
                else:
                    right = middle - 1
            else:
                result += 1

        return result

# n = len(arr1), m = len(arr2)
# Time Complexity: O(n * log(m) + m * log(m)) ==> O((n + m) * log(m))
# In Python, the sort() method sorts a list using the Timsort algorithm which is a combination
# of Merge Sort and Insertion Sort and has O(n) additional space.
# Space Complexity: O(m)


# Solution 2: Two Pointers
class Solution:
    def findTheDistanceValue(self, arr1: List[int], arr2: List[int], d: int) -> int:
        arr1.sort()
        arr2.sort()

        i, j = 0, 0
        result = 0

        while i < len(arr1) and j < len(arr2):
            if arr1[i] <= arr2[j] + d:
                if arr1[i] < arr2[j] - d:
                    result += 1
                i += 1
            else:
                j += 1

        return result + len(arr1) - i

# n = len(arr1), m = len(arr2)
# Time Complexity: O(n * log(n) + m * log(m) + n + m)
# In Python, the sort() method sorts a list using the Timsort algorithm which is a combination
# of Merge Sort and Insertion Sort and has O(n) additional space.
# Sorting two given arrays happen sequentially one after the other
# Space Complexity: O(n + m)

# TODO: Review two pointers approach is working
