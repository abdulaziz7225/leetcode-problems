"""
Problem Number: 381. Insert Delete GetRandom O(1) - Duplicates allowed
...
elements and also reporting a random element.
...
the item is present, false otherwise.
...
O(1) time complexity. Note: The test cases are generated such that getRandom will only
be called if there is at least one item in the RandomizedCollection.
...
class RandomizedCollection:
    ...
    def remove(self, val: int) -> bool:
        ...
        self.list.pop()
        self.hashmap[last_elem].add(idx)
        self.hashmap[last_elem].discard(len(self.list))
