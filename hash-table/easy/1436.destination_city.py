"""
Problem Number: 1436. Destination City
...
class Solution:
    def destCity(self, paths: List[List[str]]) -> str:
        starting_cities = set()
        for city_A, _ in paths:
            starting_cities.add(city_A)

        for _, city_B in paths:
            if city_B not in starting_cities:
                return city_B

        return ""

# n = len(paths), m = average length of cities
