"""
Problem Number: 2661. First Completely Painted Row or Column
Difficulty Level: Medium
https://leetcode.com/problems/first-completely-painted-row-or-column

********************************************************************************

You are given a 0-indexed integer array arr, and an m x n integer matrix mat. arr and
mat both contain all the integers in the range [1, m * n]. Go through each index i in
arr starting from index 0 and paint the cell in mat containing the integer arr[i].
Return the smallest index i at which either a row or a column will be completely painted
in mat.
  
Example 1:
Input: arr = [1,3,4,2], mat = [[1,4],[2,3]]
Output: 2
Explanation: The moves are shown in order, and both the first row and second column of
the matrix become fully painted at arr[2].

Example 2:
Input: arr = [2,8,7,4,1,3,5,6,9], mat = [[3,2,5],[1,4,6],[8,7,9]]
Output: 3
Explanation: The second column becomes fully painted at arr[3].
  
Constraints:
m == mat.length
n = mat[i].length
arr.length == m * n
1 <= m, n <= 10^5
1 <= m * n <= 10^5
1 <= arr[i], mat[r][c] <= m * n
All the integers of arr are unique.
All the integers of mat are unique.
"""

from typing import List


class Solution:
    def firstCompleteIndex(self, arr: List[int], mat: List[List[int]]) -> int:
        freq_map = dict()
        row_len = len(mat)
        col_len = len(mat[0])

        row_freq = [0] * row_len
        col_freq = [0] * col_len

        for i in range(row_len):
            for j in range(col_len):
                freq_map[mat[i][j]] = (i, j)

        for idx in range(len(arr)):
            i, j = freq_map[arr[idx]]
            row_freq[i] += 1
            col_freq[j] += 1
            if row_freq[i] == col_len or col_freq[j] == row_len:
                return idx

        return -1

# m = row size of matrix, n = column size of matrix
# Time Complexity: O(m * n)
# Space Complexity: O(m * n)
